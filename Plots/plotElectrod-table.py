...
class MyData:
    def __init__(self, pAcc, pStep, pScope, pSolver, pStrategy, pRun, pTime):
        ...
        self.run = pRun
        if pTime != 'Timeout' and pTime != 'Error':
    ...
    def append(self, pTime):
        if pTime != 'Timeout' and self.mean != 'Timeout' and pTime != 'Error' and self.mean != 'Error':
...
def compute(store, lines, solver, strategy, run):
    for line in lines:
        if re.match("Label: ChosenValue", line):
            ...
            StepsStr = re.search(r'Steps: \d+..\d+', line).group()
            steps = re.search(r'\d+..\d+', StepsStr).group()

            scope = 'V' + value + 'B' + ballot + 'Q' + quorum

            my_data = MyData(nAcc, steps, scope, solver, strategy, run, ms)
...
def generate_table(acc, data):
    print('\n\n\n\n\n------------' + str(acc) + '------------')
    ordered_data = sorted(data, key = lambda x: (x.acc, x.step, x.scope, x.solver, x.strategy))
    ...
    while index < len(ordered_data):
        ...
        for inner_index in range(index, index + 6):
            ...
            if column_index_element != 5:
                print('&')
                column_index_element += 1
            elif column_index_element == 5:
                print('&')
...
run = 'scope-paxos-no-message-bounded'
directory = os.getcwd(); #root of project
my_path = directory + '/Plots/results/electrodx/bounded/';


print("Caminho " + my_path)

/***************MultiPaxos with Dynamic Messages******************/
...
fact Next {
	always (nop or
		some p: Proposer | phase1A[p] or phase2A[p] or 
		preempt[p] or
...
pred phase2A [p : Proposer] {
	no m2a: M2A | m2a.bal = p.pBal
	some q : Quorum {
		// M1B stores all 1B msgs with bal = pBal sent by all acceptors from q  
		let m1B = {m1b : M1B | m1b.from in q.acceptors and m1b.bal = p.pBal} | { 
		...
		let unused = mins[Slot - Ballot.(m1B.voted).Value, 1] | { 
			...
			some m: M2A' {
				...
				maxSV in m.propSV’
				all s : Slot, v : Value | s->v in (m.propSV’ - maxSV) implies s in unused
...
pred phase2B[a: Acceptor] { 
	some m2a: M2A {
		(no a.aBal or Bal/gte[m2a.bal, a.aBal]) implies {	
			no m: M2B | m.bal = m2a.bal and m.from = a
		 	some m2b: M2B' {
				m2b.from' = a
				m2b.bal' = m2a.bal
			...
			//aVoted' = (aVoted - (a->Ballot->(m2a.propSV.Value)->Value)) + a->(m2a.bal)->(m2a.propSV)			
				...
				no m: Preempt | m.bal = a.aBal and m.from = a and m.to = m2a.from
				some m: Preempt' {
					m.to' = m2a.from
					m.bal' = a.aBal
					m.from' = a
...
check Safe for exactly 2 Value, exactly 1 Quorum, exactly 2 Acceptor, 
	exactly 2 Ballot, exactly 2 Proposer, exactly 2 Slot, 14 Message, 
	15..15 steps expect 0

assert VotedInv {
	always (all a: Acceptor, b: Ballot, s: Slot, v: Value | votedForIn[a,b,s,v] implies safeAt[b,s,v])
}

check VotedInv for exactly 2 Value, exactly 1 Quorum, exactly 2 Acceptor, 
	exactly 2 Ballot, exactly 2 Proposer, exactly 2 Slot, 14 Message, 
	15..15 steps expect 0
...
check VotedOnce for exactly 2 Value, exactly 1 Quorum, exactly 2 Acceptor, 
	exactly 1 Ballot, exactly 1 Proposer, exactly 2 Slot, 14 Message, 
	15..15 steps expect 0
...
pred safeAt[b: Ballot, s: Slot, v: Value] {
	all c: prevs[b] | some q: Quorum | all a: q.acceptors | votedForIn[a, c, s, v] or 
				wontVoteIn[a, c, s]
}
...
check Acc for 1 Value, 1 Quorum, 2 Acceptor, 2 Ballot, 3 Slot, 2 Proposer, 
	13 Message, 14..14 steps
...
assert Msg1B {
	always (all m1b: M1B | invM1B[m1b])
}

check Msg1B for 1 Value, 1 Quorum, 2 Acceptor, 2 Ballot, 3 Slot, 2 Proposer, 
	13 Message, 14..14 steps
...
pred invM2A[m: M2A] {
	...
	all m2a: M2A | m2a.bal = m.bal implies m2a = m
}	

assert Msg2A {
	always (all m2a: M2A | invM2A[m2a])
}

check Msg2A for 1 Value, 1 Quorum, 2 Acceptor, 2 Ballot, 3 Slot, 2 Proposer, 
	13 Message, 14..14 steps
...
assert Msg2B {
	always (all m2b: M2B | invM2B[m2b])
}

check Msg2B for 1 Value, 1 Quorum, 2 Acceptor, 2 Ballot, 3 Slot, 2 Proposer, 
	13 Message, 14..14 steps
...
fun events : set Event {
	nop_happens + 
	phase1A_happens.Proposer + 
	phase1B_happens.Acceptor +
	phase2B_happens.Acceptor + 
	phase2A_happens.Proposer +

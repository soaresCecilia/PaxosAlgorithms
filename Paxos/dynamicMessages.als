/***************PAXOS Dynamic Messages******************/

//Paxos with dynamic Messages
...
/*	without the next guard an occurrence of 
	Phase1A event is undistinguishable from Nop event.
	However, it does not allow duplicated M1A.
*/
	historically (no m1a: M1A | m1a.bal = b)           // theme's guard
...
pred safeNoVote[q: Quorum, b: Ballot] {
 	all a: q.acceptors {
		...
		all m1b: M1B | (m1b.acceptor = a and m1b.bal = b) 
			implies no m1b.mbal
	}
}
...
pred safeVotedValue[q: Quorum, b: Ballot, v: Value] {	
	all a: q.acceptors | some m1b: M1B {
		m1b.acceptor = a
		m1b.bal = b
	}
	some n : M1B {
		...
		all msg1b: M1B { 
			(msg1b.bal = b and msg1b.acceptor in q.acceptors) implies {
						gte[n.mbal, msg1b.mbal] or no msg1b.mbal
			}
		}
	}
}
...
pred phase2B[a: Acceptor] { 	
	...
	/*	without the next guard an occurrence of 
		Phase2B event is undistinguishable from Nop event.
		However, it does not allow duplicated M2B.
		*/
		historically (no m2b: M2B | m2b.bal = m2a.bal and m2b.acceptor = a)           // theme's guard
...
check ChosenValue for exactly 2 Value, 2 Quorum,
 exactly 3 Acceptor, exactly 2 Ballot, 8 Message, 9..9 steps


//An acceptor a has voted in v in ballot b
pred votedFor[a: Acceptor, b: Ballot, v: Value] {
	some m2b: M2B | m2b.acceptor = a and m2b.bal = b and m2b.value = v
}
...
assert votesSafe {
	always (all a : Acceptor, b : Ballot, v : Value | always (votedFor[a, b, v] implies safeAt[b, v]))
}

check votesSafe for 15..15 steps, exactly 3 Value, 
	exactly 3 Quorum, exactly 3 Acceptor, exactly 3 Ballot, 14 Message
...
check oneVote  for 15..15 steps, exactly 3 Value, 
	exactly 3 Quorum, exactly 3 Acceptor, exactly 3 Ballot, 14 Message

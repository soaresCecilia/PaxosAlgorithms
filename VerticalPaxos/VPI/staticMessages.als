/************VERTICAL PAXOS I WITH STATIC MESSAGES*********/
...
abstract sig ServerMessage extends BasicMessage  {
	bal: one Ballot
}
...
sig MsgNewBallot extends ServerMessage {
	completeBal: lone Ballot
}

sig M1A extends ServerMessage {
	prevBal: lone Ballot
}

sig M1B extends ServerMessage {
	acceptor: one Acceptor,
	mbal: lone Ballot,
	mval: lone Value
}

sig M2A extends ServerMessage {
	value: one Value
}

sig M2B extends ServerMessage {
	acceptor: one Acceptor,
	value: one Value
}

sig MsgComplete extends ServerMessage{}
...
//Next-state actions - All possible actions that change state or nop
fact Next {
	always (nop or
...
pred leaderRcvdNewReconfig[l: Leader] {
	some msgNewBallot: MsgNewBallot & MsgBox {
...
pred phase1A[l: Leader] {
	...
	no m1a: M1A & MsgBox | m1a.bal = l.id and m1a.prevBal = l.previousBal
...
//No Previous Value Chosen or Some Chosen Value
...
/* Some acceptors from some readQuorums of some previous round 
...
// All members from a current write quorum have sent a M2B
...
//If no one has voted can be propose some value

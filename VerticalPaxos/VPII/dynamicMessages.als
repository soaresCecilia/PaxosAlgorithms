/*********VERTICAL PAXOS II DYNAMIC MESSAGE ********/
...
all messages, except ClientMsg, have relation bal in common */
...
pred leaderNewReconfig[l: Leader] {
	some msgNewBallot: MsgNewBallot {
		...
		some msgNewBallot.prevBal
		no msg1a: M1A | msg1a.bal = msgNewBallot.bal 
	
		// Prepare Phase
		some m1a: M1A' {
		 	m1a.bal'= msgNewBallot.bal
			m1a.prevBal' = msgNewBallot.prevBal
			send[m1a]
		}
		previousBal' = previousBal ++ (l-> msgNewBallot.prevBal)
	}
	maxBal' = maxBal
	votes' = votes
	curBallot' = curBallot
...
pred allValuesSafeOrOneValueSafe[l: Leader] {
	some msgNew: MsgNewBallot | msgNew.bal = l.id
...
/*All acceptors from a quorum of previous view had sent a M1B
...
has voted in some value. Therefore, it is stored in safeVal because
...
pred phase2B[a: Acceptor] { 
	some m2a: M2A {
		...
		no msg2b: M2B | msg2b.bal = m2a.bal and msg2b.acceptor = a
...
//Reconfiguration is completed when Master receives a MsgComplete and sends MsgActivated
...
pred clientRequest[v: Value] {
	no msgClient: MsgClient | msgClient.value = v 
	some msgClient: MsgClient' {
		msgClient.value' = v
		send[msgClient]
	}	
...
check ChosenValue for exactly 9 Quorum, 
	...
	exactly 2 Value,  24 BasicMessage, 25..25 steps expect 0
...
check VotesSafe for exactly 9 Quorum, 
	exactly 2 Leader, exactly 3 Acceptor, exactly 3 Ballot, 
	exactly 2 Value, 14 BasicMessage, 15..15 steps expect 0
